    ...
    for  (mut player_info, mut velocity, entity) in &mut query{
        ...
        match gamepad {
            Some(gamepad) => {
                ...
                if buttons.pressed(fast_fall_button)&& !player_info.has_contacts{
        ...
        if keyboard_input.pressed(KeyCode::LShift) && !player_info.has_contacts{
            velocity.linvel.y = -30.0;
        }
...
fn move_scene_entities(
    ...
    let _res = rt.block_on(async {
        ...
        let (decompositions_sender, decompositons_reciever) = unbounded();
        decompositions_sender.send(decompositions).unwrap();

        let assets_mutex = Arc::new(Mutex::new(assets));
        let mesh_handles_mutex = Arc::new(Mutex::new(mesh_handles));
        ...
        for moved_scene_entity in &moved_scene {
            ...
            let decompositons_reciever = decompositons_reciever.clone();
            let future = iter_hierarchy(moved_scene_entity, &children, Arc::new({
                println!("running future");
                let decompositions_sender = decompositions_sender.clone();
                let decompositons_reciever = decompositons_reciever.clone();
                //let commands_sender = commands_sender.clone();
                //let commands_reciever = commands_reciever.clone();
                let moved_scene_entity = moved_scene_entity.clone();
                let assets_mutex = assets_mutex.clone();
                let mesh_handles_mutex = mesh_handles_mutex.clone();
                println!("here1");
                ...
                let mesh_handles_mutex = mesh_handles_mutex.clone();
                println!("here3");
                let mesh_handles = mesh_handles_mutex.lock().unwrap();
                println!("here4");

                let x = mesh_handles.get(entity).unwrap();

                if let Ok(mesh_handle) = mesh_handles.get(entity) {
                    println!("here5");
                    let assets = &*assets_mutex.lock().unwrap();
                    println!("here6");
                    ...
                    let mut decompositions_clone = decompositons_reciever.recv().unwrap();
                    decompositions_clone.push((entity,decomposition));
                    decompositions_sender.send(decompositions_clone).unwrap();
            ...
            println!("pushing future");
            futures.push(future);
            println!("pushed future");
        ...
        let (final_data_sender, final_data_reciever) = unbounded();
        final_data_sender.send((vec![], vec![])).unwrap();

        loop{
            let decompositions = decompositons_reciever.recv().unwrap();
            if decompositions.len() >= number_of_objects {
                decompositions_sender.send(decompositions).unwrap();
                break
            }
            else{                
                println!("there are {number_of_objects} objects and {} decompositions - sleeping", decompositions.len());
                decompositions_sender.send(decompositions).unwrap();
        ...
        let decompositions = decompositons_reciever.recv().unwrap();
        println!("there are {} decompositions", decompositions.len());

        for (entity,decomposition) in decompositions {
            //let commands_reciever = commands_reciever.clone();
            //let commands_sender = commands_sender.clone();  
            
            let final_data_sender = final_data_sender.clone();
            let final_data_reciever = final_data_reciever.clone();
            tokio::task::spawn({
                //let commands_reciever = commands_reciever.clone();
                //let commands_sender = commands_sender.clone();

                async move {
                //let mut commands = commands_reciever.recv().unwrap();
                
                let collider:Collider = decomposition.await.await.unwrap().decomp.into();

                println!("waiting to recieve final data so append can happen");
                let (mut final_entity, mut final_collider) = final_data_reciever.recv().unwrap();
                println!(" recieved final data so append will now happen");
                final_entity.push(entity);
                final_collider.push(collider);
                final_data_sender.send((final_entity, final_collider)).unwrap();
                
                //commands_sender.send(commands).unwrap();
            }});
        ...
        loop{
            let (final_entity, final_collider) = final_data_reciever.recv().unwrap();
            if final_entity.len() >= number_of_objects {
                for i in 0..final_entity.len(){
                    println!("adding collider");
                    commands.entity(final_entity[i]).insert(final_collider[i].clone());
                }
                //println!("there are {number_of_objects} objects and {} decompositions", final_entity.len());
                break
            } else {
                final_data_sender.send((final_entity, final_collider)).unwrap();
                println!("sleeping");
...
async fn iter_hierarchy(entity: Entity, children_query: &Query<'_, '_, &Children>, f: Arc<(impl Fn(Entity) + std::marker::Send + std::marker::Sync + 'async_recursion)> ) {
    let mut futures = vec![];

    (f)(entity);
    if let Ok(children) = children_query.get(entity) {
        for child in children.iter().copied() {
            futures.push(iter_hierarchy(child, &children_query, f.clone()));
        }
    }
