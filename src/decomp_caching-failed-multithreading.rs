...
pub(crate) mod decomp_caching {
     ...
     */
    pub(crate) async fn decompose(vertices: Vec<Vec3>, indices: Box<[[u32;3]]>) -> tokio::task::JoinHandle<RenderedDecomp> {

        let cache = load_cache();

        match check_if_already_in_list(&vertices, &indices,&cache) {
            Some(item) => return tokio::task::spawn(async move{item}),
            None => {
                println!("New item! About to decompose shape with {} vertexes",vertices.len());
                

                let rendered_decomp = tokio::spawn({
                    async move {
                    //let indices = indices.clone();
                    let decomposition = Collider::convex_decomposition_with_params(&vertices, &indices, &bevy_rapier3d::prelude::VHACDParameters { concavity:0.01, ..default() });
