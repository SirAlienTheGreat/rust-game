...
fn move_scene_entities(
    ...
    let _res = rt.block_on(async {
        ...
        decompositions_sender.send(decompositions);

        let (commands_sender, commands_reciever) = unbounded();
        commands_sender.send(commands);
        ...
        for (entity,decomposition) in decompositons_reciever.recv().unwrap() {
            let mut commands = commands_reciever.recv().unwrap();
